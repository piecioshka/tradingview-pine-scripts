...
- **Imbalans kupna** — wolumen kupna na poziomie `P` ≥ *mnożnik* × wolumen sprzedaży o poziom **niżej** (`P-1`).
- **Imbalans sprzedaży** — wolumen sprzedaży na poziomie `P` ≥ *mnożnik* × wolumen kupna o poziom **wyżej** (`P+1`).
...
- **Mitygacja wg zamknięcia** — strefa testowana dopiero, gdy *zamknięcie* wejdzie w strefę (wyłączone = liczy się knot `high`/`low`, czulsze).
...
**Sygnał jest mocniejszy, gdy zgadza się z resztą:** kierunkiem delty/CVD (wskaźniki *Delta FP Table* / *Histogram*), wolumenem i kluczowymi poziomami z wykresu.
...
- Trzymane są ostatnie *N* stref (`Maks. liczba stref`) — bardzo stare są usuwane.
