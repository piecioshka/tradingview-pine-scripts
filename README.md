...
Overbalance - market geometry popularized by Łukasz Stefanik. The largest correction of the running trend becomes the reference: while new corrections stay below it the trend is healthy, and the moment price travels further than the reference away from the trend extreme, the trend is broken. Corrections are measured between confirmed swings (`ta.pivothigh` / `ta.pivotlow`) and anything below the noise filter is skipped - the method treats moves under 40 points on DAX as noise, and the filter can also be set in percent or ATR multiples for other instruments. The threshold line sits one reference correction away from the trend extreme, which is where the method puts the stop loss, and the break is detected intrabar rather than on the next confirmed swing. The geometry is a grid of rectangles, each exactly one reference overbalance tall, stacked from the trend extreme in the direction a break would travel - the first one is the break zone, and the rectangle currently holding price gets a stronger fill, so the chart reads off how many whole overbalances the market has travelled from the extreme. Correction boxes mark every measured correction with the reference one highlighted, plus signal markers, a label with the broken reference size, and an alert condition.
